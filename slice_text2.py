# This code now achieves an average processing speed of around 0.25 sec per images
# Next step: rotate it 90 degrees and see which one has the highest boxes with 0
# degrees rotation. This data is needed because for some reason, the +-90 images
# has lower box count than the normal orientation. I need this to determine the 
# general text orientation on a page so as to crop out the best representative of
# the page.
...
def process_with_east(input_image_path, crop_width, crop_height):
    ...
    bin1, im1, count1 = east_predict(scores1, geometry1, image)
    bin2, im2, count2 = east_predict(scores2, geometry2, image)
    if count1 >= count2:
        return bin1, im1
    else:
        return bin2, im2
